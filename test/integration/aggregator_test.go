...
func TestAggregatedConfigMaps(t *testing.T) {
	...
	t.Run("list merges all clusters and stamps cluster metadata", func(t *testing.T) {
		...
		require.Equal(t, 2, count["shared"], "duplicates across clusters are allowed in list results")
		require.Equal(t, 1, count["only-a"])
		require.Equal(t, 1, count["only-b"])
	})
	...
	t.Run("list filtered by virtual cluster label", func(t *testing.T) {
		...
		require.False(t, names["only-b"], "filter must exclude other clusters")
	})
